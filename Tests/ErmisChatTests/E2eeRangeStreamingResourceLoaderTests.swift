...
final class E2eeRangeStreamingResourceLoaderTests: XCTestCase {
    ...
    func testTelemetrySummaryContainsOnlyFixedCountersAndCategories() {
        ...
        telemetry.recordGrantWait(transport: .priority, latency: 0.025)
        telemetry.recordGrantWait(transport: .continuation, latency: 0.030)
        telemetry.recordTransportStarted(transport: .priority, ciphertextBytes: 2_048)
        telemetry.recordTransportStarted(transport: .continuation, ciphertextBytes: 4_096)
        telemetry.recordTransportHeader(transport: .priority, latency: 0.050)
        telemetry.recordTransportHeader(transport: .continuation, latency: 0.060)
        telemetry.recordTransportFirstChunk(transport: .priority, latency: 0.075)
        telemetry.recordTransportFirstChunk(transport: .continuation, latency: 0.085)
        telemetry.recordTransportCompleted(transport: .priority, latency: 0.125)
        telemetry.recordTransportCompleted(transport: .continuation, latency: 0.150)
        ...
        XCTAssertTrue(summary.contains("priority_grant_wait_ms=25"))
        XCTAssertTrue(summary.contains("continuation_grant_wait_ms=30"))
        XCTAssertTrue(summary.contains("priority_header_ms=50"))
        XCTAssertTrue(summary.contains("continuation_header_ms=60"))
        XCTAssertTrue(summary.contains("priority_first_chunk_ms=75"))
        XCTAssertTrue(summary.contains("continuation_first_chunk_ms=85"))
        XCTAssertTrue(summary.contains("priority_transport_ms=125"))
        XCTAssertTrue(summary.contains("continuation_transport_ms=150"))
        XCTAssertTrue(summary.contains("max_active_priority_transports=1"))
        XCTAssertTrue(summary.contains("max_active_continuation_transports=1"))
        XCTAssertTrue(summary.contains("max_priority_cipher_request_bytes=2048"))
        XCTAssertTrue(summary.contains("max_continuation_cipher_request_bytes=4096"))
