...
open class MessageListViewController: _ViewController,
                                      UIProvider,
                                      MessageListScrollOverlayDataSource,
                                      ...
                                      VoiceRecordingAttachmentPresentationViewDelegate
{
    ...
    public weak var dataSource: MessageListViewControllerDataSource? {
        didSet {
    ...
    public weak var delegate: MessageListViewControllerDelegate?

    /// The root object representing the Ermis Chat.
    public var client: ErmisClient!

    /// The router object that handles navigation to other view controllers.
    open lazy var router: MessageListRouter = components
        .messageListRouter
    ...
    open private(set) lazy var dateOverlayView: MessageListScrollOverlayView = {
    ...
    open var listViewBottomConstraint: NSLayoutConstraint?
    ...
    private var isJumpToUnreadEnabled: Bool {
        ...
        return isEnabled && delegate.messageListShouldShowJumpToUnread(self)
    }
    ...
    public lazy var dateSeparatorFormatter = formatters.messageDateSeparator
    ...
    override open func setUpUI() {
        ...
        listView.pin(anchors: [.top, .leading, .trailing], to: view)
        listViewBottomConstraint = listView.bottomAnchor.pin(equalTo: view.bottomAnchor)
        ...
        listView.contentInset = .init(top: listView.defaultContentInsetTop, left: 0, bottom: 8, right: 0)
        ...
        typingIndicatorView.topAnchor.pin(equalTo: listView.bottomAnchor).isActive = true
    ...
    open func cellLayoutOptionsForMessage(at indexPath: IndexPath) -> MessageLayoutOptions {
        dataSource?.messageListVC(self, messageLayoutOptionsAt: indexPath) ?? .init()
    }

    /// Returns the content view class for the message at given `indexPath`
    open func cellContentClassForMessage(at indexPath: IndexPath) -> MessageContentView.Type {
        components.messageContentView
    }

    /// Returns the attachment view injector for the message at given `indexPath`
    open func customCellViewInjectorClassForMessage(at indexPath: IndexPath) -> CustomCellViewInjector.Type? {
        ...
        return components.customCellViewCatalog.customCellViewInjectorClassFor(
            message: message,
    ...
    open func updateJumpToUnreadButtonVisibility(animated: Bool = true) {
        guard isJumpToUnreadEnabled else {
            return
        }
    ...
    @objc func didTapDiscardJumpToUnreadButton() {
        delegate?.messageListDidDiscardUnreadMessages(self)
    }
    ...
    @objc open func handleTap(_ gesture: UITapGestureRecognizer) {
        delegate?.messageListVC(self, didTapOnMessageListView: listView, with: gesture)
    ...
    open func gestureRecognizerShouldBegin(_ gesture: UIGestureRecognizer) -> Bool {
        ...
        guard let indexPath = listView.indexPathForRow(at: location),
              let cell = listView.cellForRow(at: indexPath) as? MessageCell else {
            return false
        }
    ...
    open func didSelectMessageCell(at indexPath: IndexPath) {
        guard
            ...
            dataSource?.channel(for: self)?.isGuess != true
        else { return }
        ...
        let reactionsController: MessageReactionsPickerViewController? = {
    ...
    open func showTypingIndicator(typingUsers: [ChatUser]) {
        ...
        listView.pin(anchors: [.top, .leading, .trailing], to: view)
        listViewBottomConstraint?.constant = -typingIndicatorViewHeight
    }
    ...
    open func hideTypingIndicator() {
        ...
        typingIndicatorView.isHidden = true
        listView.pin(anchors: [.top, .leading, .trailing], to: view)
        listViewBottomConstraint?.constant = 0
    }
    ...
    func shouldShowDateSeparator(forMessage message: ChatMessage, at indexPath: IndexPath) -> Bool {
        ...
        guard let previousMessage = dataSource?.messageListVC(self, messageAt: previousIndexPath) else {
    ...
    open func messageActions(forDebouncedMessage message: ChatMessage) -> [MessageActionItem] {
        ...
        return [
            ...
            EditActionItem(
                ...
                action: { [weak self] item in
                    ...
                    self.delegate?.messageListVC(self, didTapOnAction: item, for: message)
                }
            ),
            DeleteActionItem(
    ...
    public func jumpToMessage(id: MessageId, animated: Bool = true, onHighlight: ((IndexPath) -> Void)? = nil) {
        ...
        delegate?.messageListVC(self, shouldLoadPageAroundMessageId: id) { [weak self] error in
            if let error = error {
    ...
    open func highlightCell(at indexPath: IndexPath) {
        guard let cell = listView.cellForRow(at: indexPath) as? MessageCell else {
            return
        }
        let previousBackgroundColor = cell.messageContentView?.backgroundColor
        let highlightColor = theme.colors.surfaceContainerHigh
    ...
    internal func jumpToFirstPage() {
        delegate?.messageListVCShouldLoadFirstPage(self)
        scrollToBottomButton.isHidden = true
    ...
    private func getCurrentUnreadMessageId(completion: @escaping (MessageId?) -> Void) {
        ...
        delegate?.messageListVC(self, shouldLoadPageAroundMessageId: lastReadMessageId) { error in
    ...
    open func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let message = dataSource?.messageListVC(self, messageAt: indexPath)
        let cell: MessageCell = listView.dequeueReusableCell(
            contentViewClass: cellContentClassForMessage(at: indexPath),
            customCellViewInjectorType: customCellViewInjectorClassForMessage(at: indexPath),
        ...
        cell.setDecoration(for: .footer, decorationView: delegate?.messageListVC(self, footerViewForMessage: message, at: indexPath))
...
//        imagePrefetcher.stopPrefetching(with: urls)
    ...
    open func scrollViewDidScroll(_ scrollView: UIScrollView) {
        delegate?.messageListVC(self, scrollViewDidScroll: scrollView)
    ...
    // MARK: - MessageListScrollOverlayDataSource

    open func scrollOverlay(
        _ overlay: MessageListScrollOverlayView,
    ...
    // MARK: - MessageActionsViewControllerDelegate

    open func messageActionsVC(
        _ vc: MessageActionsViewController,
        message: ChatMessage,
        didTapOnActionItem actionItem: MessageActionItem
    ) {
        delegate?.messageListVC(self, didTapOnAction: actionItem, for: message)
    }

    open func messageActionsVCDidFinish(_ vc: MessageActionsViewController) {
        messageActionsVC = nil
        dismiss(animated: true)
    }

    // MARK: - MessageContentViewDelegate
    ...
    open func messageContentViewDidTapAtShowEditedHistory(_ indexPath: IndexPath?) {
        guard let indexPath = indexPath else {
            return log.error("IndexPath is not available")
        }
        ...
        let editedMessageListVC = components.editedMessageListViewController.init()
        editedMessageListVC.messageController = messageController
        self.present(editedMessageListVC, animated: false)
    }
    ...
    open func galleryMessageContentView(
        ...
        guard let message = dataSource?.messageListVC(self, messageAt: indexPath) else { return }
