...
open class ThreadViewController: _ViewController,
                                 UIProvider,
    ...
    open var messageController: MessageController!
    ...
    open lazy var headerView: ThreadHeaderView = components
    ...
    public func messageListVC(
        _ vc: MessageListViewController, shouldLoadPageAroundMessageId messageId: MessageId,
        _ completion: @escaping ((Error?) -> Void)
    ) {
        messageController.loadPageAroundReplyId(messageId, completion: completion)
    }

    open func messageListVCShouldLoadFirstPage(_ vc: MessageListViewController) {
    ...
    open func eventsController(_ controller: EventsController, didReceiveEvent event: Event) {
        ...
        case let event as TypingEvent:
            guard event.parentId == messageController.messageId && event.user.userId != client.currentUserId else { return }
            if event.isTyping {
    ...
    private func getMessages(from messageController: MessageController) -> LazyCachedMapCollection<ChatMessage> {
