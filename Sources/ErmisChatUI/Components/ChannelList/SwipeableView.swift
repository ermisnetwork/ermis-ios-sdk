...
public protocol SwipeableViewDelegate: AnyObject {
    ...
    /// `ChannelListCollectionViewCell` can have swipe to delete / reveal action buttons on the cell.
    ///
...
open class SwipeableView: _View, ComponentsProvider, UIGestureRecognizerDelegate {
        ...
        case .began:
            ...
            if !isOpen {
                ...
                actionButtons.forEach {
                    actionItemsStackView.addArrangedSubview($0)
                }
    ...
    /// Show the stackView with buttons.
    open func open() {
        actionStackViewWidthConstraint?.constant = actionItemsStackView.idealWidth
    }
}
