...
open class MessageActionsTransitionController: NSObject, UIViewControllerTransitioningDelegate,
    ...
    public private(set) weak var messageListVC: MessageListViewController?
    ...
    open var selectedMessageContentViewFrame: CGRect? {
        ...
        frame.size.height -= reactionsBubbleHeight == 0 ? 0 : 14
        return frame
    }

    /// The message cell that displays the selected message.
    open var selectedMessageCell: MessageCell? {
        messageListVC?
            .listView
            .visibleCells
    ...
    public required init(messageListVC: MessageListViewController?) {
        super.init()
    ...
    open func animatePresent(using transitionContext: UIViewControllerContextTransitioning) {
        guard
            let toVC = transitionContext.viewController(forKey: .to) as? MessagePopupViewController,
    ...
    open func animateDismiss(using transitionContext: UIViewControllerContextTransitioning) {
        guard
            let fromVC = transitionContext.viewController(forKey: .from) as? MessagePopupViewController,
    ...
    open func makeMessageContentView(fromOriginalView originalView: MessageContentView) -> MessageContentView {
        let messageViewType = type(of: originalView)
        let messageAttachmentInjectorType = originalView.customCellViewInjector.map { type(of: $0) }
        ...
        messageView.setUpUIIfNeeded(options: messageLayoutOptions, customCellViewInjectorType: messageAttachmentInjectorType)
