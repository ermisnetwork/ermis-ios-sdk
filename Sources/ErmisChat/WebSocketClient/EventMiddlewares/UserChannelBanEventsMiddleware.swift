    ...
    func handle(event: Event, session: DatabaseSession) -> Event? {
        do {
            ...
            case let userBannedEvent as UserBannedEventDTO:
                guard let memberDTO = session.member(userId: userBannedEvent.user.userId, cid: userBannedEvent.cid) else {
                    throw ClientError.MemberDoesNotExist(userId: userBannedEvent.user.userId, cid: userBannedEvent.cid)
                }
            ...
            case let userUnbannedEvent as UserUnbannedEventDTO:
                guard let memberDTO = session.member(userId: userUnbannedEvent.user.userId, cid: userUnbannedEvent.cid) else {
                    throw ClientError.MemberDoesNotExist(userId: userUnbannedEvent.user.userId, cid: userUnbannedEvent.cid)
                }
