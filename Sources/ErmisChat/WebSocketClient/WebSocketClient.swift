...
class WebSocketClient {
    ...
    @Atomic private(set) var connectionState: WebSocketConnectionState = .initialized {
        didSet {
            engineQueue.async { [connectionState, weak pingController] in
    ...
    private let engineQueue: DispatchQueue = .init(label: "network.ermis.chat.core.web_socket_engine_queue", qos: .userInitiated)
...
extension Notification.Name {
    ...
    static let NewEventReceived = Notification.Name("network.ermis.chat.core.new_event_received")
}

extension Notification {
    private static let eventKey = "network.ermis.chat.core.event_key"
...
struct WebSocketErrorContainer: Decodable {
    ...
    let error: WebSocketErrorPayload
}
