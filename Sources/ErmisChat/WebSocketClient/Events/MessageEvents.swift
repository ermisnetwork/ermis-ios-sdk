...
/// Triggered when a message is updated.
public struct MessagePinnedEvent: ChannelSpecificEvent {
    /// The use who updated the message.
    public let user: ChatUser
    ...
    /// The message is pinned or unpin.
...
class MessagePinnedEventDTO: EventDTO {
    let user: UserPayload
    let cid: ChannelId
    let parentCid: ChannelId?
    let message: MessagePayload
    ...
    init(from response: EventPayload) throws {
        ...
        createdAt = try response.value(at: \.createdAt)
        payload = response
        isPinned = response.eventType == "message_pinned"
    }
...
class MessageDeletedEventDTO: EventDTO {
    ...
    init(from response: EventPayload) throws {
        ...
        createdAt = try response.value(at: \.createdAt)
        payload = response
        hardDelete = true//response.hardDelete
    }
