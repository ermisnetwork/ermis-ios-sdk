        ...
        __cached.mutate { value in
            ...
            if let context = context {
                guard persistentStoreIdentifier == context.persistentStoreCoordinator?.persistentStores.first?.identifier else {
                    ...
                    This can happen if a snapshot of data (ChatMessage, ChatUser, Channel) was kept in memory after Database is wiped (for example, in the event of logging in with a new user).
                    If you're sure there are no references to models from another session, please report this stack trace to Ermis Team.
                    """
                    log.error(message)
