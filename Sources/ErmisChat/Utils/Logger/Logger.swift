...
public struct LogSubsystem: OptionSet {
    ...
    /// The subsystem responsible for call.
...
public enum LogConfig {
    ...
    ///
    /// - Important: Other options in `ErmisClientConfig.Logging` will not take affect if this is changed.
    ...
    private static var queue = DispatchQueue(label: "network.ermis.logconfig")

    /// Logger instance to be used by ermisChat.
    ///
...
public class Logger {
    ...
    private let loggerQueue = DispatchQueue(label: "network.ermis.logger")
    ...
    public func assert(
        ...
        if ErmisRuntimeCheck.assertionsEnabled {
...
extension Data {
    ...
    var debugPrettyPrintedJSON: String? {
        ...
        if let jsonObject = try? JSONSerialization.jsonObject(with: self, options: [.allowFragments]) as? [String: AnyObject],
           let prettyPrintedData = try? JSONSerialization.data(withJSONObject: jsonObject, options: [.prettyPrinted]) {
            ...
            return "Can not pretty print data \(self)"
        }
    }
}
