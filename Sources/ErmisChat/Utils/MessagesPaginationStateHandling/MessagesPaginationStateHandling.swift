...
class MessagesPaginationStateHandler: MessagesPaginationStateHandling {
    private let queue = DispatchQueue(label: "network.ermis.messages-pagination-state-handler")
    ...
    func end(pagination: MessagesPagination, with result: Result<[MessagePayload], Error>) {
        ...
        let isSortedFromOldestToLastest = (messages.first?.createdAt ?? Date()) < (messages.last?.createdAt ?? Date())
        let oldestFetchedMessage = isSortedFromOldestToLastest ? messages.first : messages.last
        let newestFetchedMessage = isSortedFromOldestToLastest ? messages.last : messages.first
    ...
    private func calculateHasLoadedAllMessagesBasedOnTheLocation(
        ...
        let midPoint: Int = Int(round(midIndex))
