...
/// A type representing a unique identifier of a `Channel`.
///
...
public struct ChannelId: Hashable, CustomStringConvertible {
    ...
    ///
    public init(type: ChannelType, projectId: String, id: String) {
        rawValue = type.rawValue + "\(Self.separator)" + projectId + "\(Self.separator)" + id
    }
    ...
    public init(cid: String) throws {
        ...
        guard
            ...
            !channelPair[2].replacingOccurrences(of: " ", with: "").isEmpty
        else {
...
public extension ChannelId {
    ...
    var id: String {
        let channelPair = rawValue.components(separatedBy: Self.separatorCharSet)
        return channelPair.suffix(2).joined(separator: ":")
    }
}
