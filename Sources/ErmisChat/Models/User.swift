...
public class ChatUser {
    ...
    ///
    /// You need to enable multi-tenancy if you want to use this, else it'll be empty.
    ...
    /// Check two chat user equal.
    /// An easy way for subclass to comform to `Equatable` protocol
    public func isEqual(to chatUser: ChatUser) -> Bool {
        id == chatUser.id
        && projectId == chatUser.projectId
        && name == chatUser.name
        && imageURL == chatUser.imageURL
        && isOnline == chatUser.isOnline
        && isBanned == chatUser.isBanned
        && isFlaggedByCurrentUser == chatUser.isFlaggedByCurrentUser
        && userRole == chatUser.userRole
        && userCreatedAt == chatUser.userCreatedAt
        && userDeactivatedAt == chatUser.userDeactivatedAt
        && lastActiveAt == chatUser.lastActiveAt
        && teams == chatUser.teams
    }
}
