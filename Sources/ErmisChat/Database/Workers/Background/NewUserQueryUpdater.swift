        ...
        database.backgroundReadOnlyContext.perform { [weak self] in
            ...
            do {
                ...
                let updatedQueries = try self?.queriesObserver.items.map {
                    ...
                    try $0.asUserListQueryWithUpdatedFilter(filterToAdd: .equal("userId",
                                                                                to: userDTO.userId),
                ...
                updatedQueries?.forEach {
                    self?.userListUpdater.update(userListQuery: $0, projectId: userDTO.projectId) { error in
                        if let error = error {
...
private extension UserListQueryDTO {
    func asUserListQueryWithUpdatedFilter(
        filterToAdd filter: Filter<UserListFilterScope>, projectId: String?) throws -> UserListQuery {
        let originalFilter = try JSONDecoder.default.decode(Filter<UserListFilterScope>.self, from: filterJSONData)
            return UserListQuery(filter: originalFilter, projectId: projectId)
