...
extension MessageReactionDTO {
    ...
    static func loadOrCreate(
        ...
        let reactionId = createId(userId: user.userId, messageId: message.id, type: type)
...
extension NSManagedObjectContext {
    ...
    func saveReaction(
        ...
        if let cid = messageDTO.cid {
            projectId = (try? ChannelId(cid: cid).projectId) ?? ""
        }
        let dto = MessageReactionDTO.loadOrCreate(
            message: messageDTO,
            type: payload.type,
            user: try saveUser(payload: payload.user, projectId: projectId),
            context: self,
            cache: cache
        )
...
extension MessageReactionDTO {
    ...
    func asModel() throws -> MessageReaction {
