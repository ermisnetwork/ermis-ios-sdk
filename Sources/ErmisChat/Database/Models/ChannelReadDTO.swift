...
class ChannelReadDTO: NSManagedObject {
    ...
    static func fetchRequest(userId: String) -> NSFetchRequest<ChannelReadDTO> {
        ...
        request.predicate = NSPredicate(format: "user.id CONTAINS %@", userId)
        return request
    }

    static func fetchRequest(for cid: ChannelId, userId: String) -> NSFetchRequest<ChannelReadDTO> {
        let request = NSFetchRequest<ChannelReadDTO>(entityName: ChannelReadDTO.entityName)
        request.predicate = NSPredicate(format: "channel.cid == %@ && user.id CONTAINS %@", cid.rawValue, userId)
        return request
    }
    ...
    static func loadOrCreate(
        ...
        new.user = UserDTO.loadOrCreate(id: userId, projectId: cid.projectId, context: context, cache: cache)
        return new
    }

    /// Snapshots the current state of `ChannelReadDTO` and returns an immutable model object from it.
    func asModel() throws -> ChannelRead { try .create(fromDTO: self) }
}
...
extension NSManagedObjectContext {
    func saveChannelRead(
        ...
        dto.user = try saveUser(payload: payload.user, projectId: cid.projectId)
    ...
    private func markMessagesFromCurrentUserAsRead(
        ...
        let messages = MessageDTO.loadCurrentUserMessages(
            ...
            createdAtThrough: read.lastReadAt?.bridgeDate ?? Date(),
            context: self
        )
...
extension ChannelRead {
    fileprivate static func create(fromDTO dto: ChannelReadDTO) throws -> ChannelRead {
        try .init(
            lastReadAt: dto.lastReadAt?.bridgeDate ?? Date(),
