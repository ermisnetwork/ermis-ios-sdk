...
class BackgroundDatabaseObserver<Item, DTO: NSManagedObject> {
    ...
    private let queue = DispatchQueue(label: "network.ermis.list-database-observer", qos: .userInitiated, attributes: .concurrent)
    ...
    init(
        ...
        operationQueue.name = "network.ermis.database-observer"
