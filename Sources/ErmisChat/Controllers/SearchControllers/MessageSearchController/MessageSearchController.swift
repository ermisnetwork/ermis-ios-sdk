...
public extension ErmisClient {
    ...
    ///
    func messageSearchController() -> MessageSearchController {
        .init(client: self)
    }
}

/// `MessageSearchController` is a controller class which allows observing a list of messages based on the provided query.
public class MessageSearchController: DataController, DelegateCallable, DataStoreProvider {
    ...
    var multicastDelegate: MulticastDelegate<MessageSearchControllerDelegate> = .init() {
        didSet {
    ...
    public func search(text: String, completion: ((_ error: Error?) -> Void)? = nil) {
        ...
        let query = MessageSearchQuery(
            channelFilter: .joinedChannels(memberId: currentUserId,
...
extension MessageSearchController {
    /// Set the delegate of `MessageSearchController` to observe the changes in the system.
    public weak var delegate: MessageSearchControllerDelegate? {
        get { multicastDelegate.mainDelegate }
        set { multicastDelegate.set(mainDelegate: newValue) }
    }
}

/// `MessageSearchController` uses this protocol to communicate changes to its delegate.
public protocol MessageSearchControllerDelegate: DataControllerStateDelegate {
