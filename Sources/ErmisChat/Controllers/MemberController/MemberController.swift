...
public extension ErmisClient {
    ...
    /// - Returns: A new instance of `ChannelMemberController`.
    func memberController(userId: UserId, in cid: ChannelId) -> ChannelMemberController {
        .init(userId: userId, cid: cid, client: self)
    }
}

/// `ChannelMemberController` is a controller class which allows mutating and observing changes of a specific chat member.
///
public class ChannelMemberController: DataController, DelegateCallable, DataStoreProvider {
    ...
    /// `Combine` publishers.
    public var member: ChannelMember? {
    ...
    var multicastDelegate: MulticastDelegate<ChannelMemberControllerDelegate> = .init() {
        didSet {
    ...
    /// Creates a new `ChannelMemberController`
    /// - Parameters:
    ///   - userId: The user identifier.
...
public extension ChannelMemberController {
...
extension ChannelMemberController {
    struct Environment {
        ...
        ) -> EntityDatabaseObserverWrapper<ChannelMember, MemberDTO> = EntityDatabaseObserverWrapper.init
    }
}

public extension ChannelMemberController {
    /// Set the delegate of `MemberController` to observe the changes in the system.
    var delegate: ChannelMemberControllerDelegate? {
...
/// `ChannelMemberControllerDelegate` uses this protocol to communicate changes to its delegate.
public protocol ChannelMemberControllerDelegate: DataControllerStateDelegate {
    /// The controller observed a change in the `ChannelMember` entity.
...
public extension ChannelMemberControllerDelegate {
    func memberController(
        _ controller: ChannelMemberController,
        didUpdateMember change: EntityChange<ChannelMember>
    ) {}
