...
// MARK: - JSONDecoder Ermis

final class ErmisJSONDecoder: JSONDecoder {
    ...
    init(dateFormatter: ISO8601DateFormatter, dateCache: NSCache<NSString, NSDate>) {
        ...
        dateDecodingStrategy = .custom { [weak self] decoder throws -> Date in
            ...
            if let date = DateFormatter.Ermis.rfc3339Date(from: dateString) {
                self?.dateCache.setObject(date.bridgeDate, forKey: dateString as NSString)
                return date
            }
...
extension JSONDecoder {
    ...
    static var `default`: JSONDecoder = ermis

    /// A Ermis Chat JSON decoder.
    static let ermis: ErmisJSONDecoder = {
        ErmisJSONDecoder()
    }()
}

// MARK: - JSONEncoder Ermis

extension JSONEncoder {
    /// A default `JSONEncoder`.
    static var `default`: JSONEncoder = ermis
    /// A default gzip `JSONEncoder`.
    static var defaultGzip: JSONEncoder = ermisGzip

    /// A Ermis Chat JSON encoder.
    static let ermis: JSONEncoder = {
        let encoder = JSONEncoder()
        encoder.dateEncodingStrategy = .ermis
        return encoder
    }()

    /// A Ermis Chat JSON encoder with a gzipped content.
    static let ermisGzip: JSONEncoder = {
        let encoder = JSONEncoder()
...
extension JSONEncoder.DateEncodingStrategy {
    /// A Ermis encoding for the custom ISO8601 date.
    static var ermis: JSONEncoder.DateEncodingStrategy {
        .custom { date, encoder throws in
            var container = encoder.singleValueContainer()
            try container.encode(DateFormatter.Ermis.rfc3339DateString(from: date))
        }
    }
}
