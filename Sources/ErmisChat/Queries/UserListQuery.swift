...
public extension FilterKey where Scope: AnyUserListFilterScope {
    ...
    /// A filter key for matching the `projectId` value.
    static var projectId: FilterKey<Scope, String> { "projectId" }
...
public struct UserListQuery: Encodable {
    ...
    /// A pagination.
    public var pagination: UserPagination?
    ...
    public init(
        ...
        self.filter = filter
        self.sort = sort
        pagination = UserPagination(page: 1, pageSize: pageSize)
        self.name = name
...
public extension UserListQuery {
    ...
    static func user(withID userId: UserId, projectId: String) -> Self {
        .init(filter: .and(
            [.equal(.id, to: userId + projectId),
             .equal(.projectId, to: projectId)]
        ), projectId: projectId)
    }

    /// Builds `UserListQuery` for a list user with the provided `userIds`
    /// - Parameter userId: The user identifier
    /// - Returns: `UserListQuery` for list user
    static func users(withIDs userIds: [UserId], projectId: String) -> Self {
        .init(filter: .in(.id, values: userIds.map({ $0 + projectId })) , projectId: projectId)
    }
    ...
    /// - Returns: The query.
    static func search(term: String?, showBlocked: Bool = true, projectId: String) -> Self {
        var query = UserListQuery(sort: [.init(key: .name, isAscending: true)], name: term, projectId: projectId)
        if let term = term, !term.isEmpty {
            query.filter = .and([
                ...
                .or([
                    ...
                    .contains(.name, value: term),
                    .contains(.id, value: term)
                ]),
            ])
        } else {
            query.filter = .and([
