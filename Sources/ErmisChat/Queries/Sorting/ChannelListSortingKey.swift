...
public struct ChannelListSortingKey: SortingKey, Equatable {
    ...
    /// Sort channels by the lisPinned state..
    public static let isPinned = Self(
        keyPath: \.isPinned,
        localKey: #keyPath(ChannelDTO.isPinned),
        remoteKey: ChannelCodingKeys.isPinned.rawValue
    )
    ...
    public static func custom<T>(keyPath: KeyPath<Channel, T>, key: String) -> Self {
        .init(keyPath: keyPath, localKey: nil, remoteKey: key)
    }

    let keyPath: PartialKeyPath<Channel>
    let localKey: String?
    ...
    init<T>(keyPath: KeyPath<Channel, T>, localKey: String?, remoteKey: String) {
...
extension Array where Element == Sorting<ChannelListSortingKey> {
    var runtimeSorting: [SortValue<Channel>] {
        var requiresRuntime = false
        let sortValues: [SortValue<Channel>] = compactMap {
