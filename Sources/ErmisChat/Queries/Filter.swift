...
public extension Filter {
    ...
    static func noneAnd(_ filters: [Filter]) -> Filter {
        .init(operator: .noneAnd, value: filters)
    }
}
...
public extension Filter {
    ...
    /// None
    static func none<Value: Encodable>(_ key: FilterKey<Scope, Value>, value: Value) -> Filter {
        .init(
            operator: .none,
            key: key,
            value: value,
    ...
    /// None
    static func none<Value: Encodable>(_ key: FilterKey<Scope, Value>, values: [Value]) -> Filter {
        .init(
            operator: .none,
            key: key,
            value: values,
...
extension Filter: CustomStringConvertible {
    public var description: String {
        ...
        case .none:
            let filters = value as? [Filter] ?? []
            return filters.map(\.description).joined(separator: " ")
        case .noneAnd:
            let filters = value as? [Filter] ?? []
            return "(" + filters.map(\.description).joined(separator: ") AND (") + ")"
        }
...
extension Filter: Codable {
    public func encode(to encoder: Encoder) throws {
        if self.operator.isGroupOperator {
            if let filters = value as? Array<Filter> {
                for filter in filters {
                    if let key = filter.key {
                        try [key: AnyEncodable(filter.value)].encode(to: encoder)
                    }
                }
                return
            }
            ...
            return
                ...
                try [key: AnyEncodable(value)].encode(to: encoder)
                return
        } else {
...
extension String {
    ...
    var isGroupOperator: Bool {
        let groupOperators: [FilterOperator] = [.and, .or, .nor, .noneAnd]
